...
class MyKMeans():
    def __init__ (self, k, max_iterations, algorithm, stop_mode = 'default'):
        '''
            stop_mode: 
                'default':             Uses all 3 conditions
                'sse_increase':        Stops only if SSE increases
                'centroid_stable':     Stops only if centroids are stable
                'max_iterations':      Stops only if max iterations reached
        '''
        self.k = k
    ...
    def fit(self, data):
        ...
        for i in range(self.max_iterations):
            ...
            if self.stop_mode in ['default', 'sse_increase']:
            ...
            if self.stop_mode in ['default', 'centroid_stable']:
